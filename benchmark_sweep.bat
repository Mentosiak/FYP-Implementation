...
echo.
echo Benchmark sweep completed successfully.
goto :end
...
docker run --rm %GPU_ARGS% -v %cd%:/workspace %IMAGE% python train_supervised_limited.py --config %SUP_CFG%
if errorlevel 1 exit /b 1

docker run --rm %GPU_ARGS% -v %cd%:/workspace %IMAGE% python train_ssl.py --config %SSL_CFG%
if errorlevel 1 exit /b 1

docker run --rm %GPU_ARGS% -v %cd%:/workspace %IMAGE% python compare_methods.py --supervised-config %SUP_CFG% --ssl-config %SSL_CFG%
if errorlevel 1 exit /b 1
